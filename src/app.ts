...
function setSunsetJob(day: when) {
    return setCronJob(getSunset, day, () => {
        ...
        console.log('statuses/update', { status: `Dobrou noc, právě zapadlo slunce. Noc potrvá ${hours} hodin a ${minutes} minut a skončí v ${DateTime.fromJSDate(getSunrise(LAT, LON, DateTime.local().plus({ days: 1 }).toJSDate())).toFormat('hh:mm')}.` });
...
function setSunriseJob(day: when) {
    return setCronJob(getSunrise, day, () => {
        ...
        console.log('statuses/update', { status: `Dobré ráno, právě vyšlo slunce. Den potrvá ${hours} hodin a ${minutes} minut a skončí v ${DateTime.fromJSDate(getSunset(LAT, LON)).toFormat('hh:mm')}.` });
